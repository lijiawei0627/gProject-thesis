...
export default {
  ...
  methods: {
    ...
    middleTouchEnd (e) {
      ...
      this.$refs.lyricList.$el.style[transform] = `translate3d(${offsetWidth}px,0,0)`
      this.$refs.lyricList.$el.style[transitionDuration] = `${transitionTime}ms`
      this.$refs.middleL.style.opacity = opacity
      this.$refs.middleL.style[transitionDuration] = `${transitionTime}ms`
      this.touch.initialed = false
    },
    // 实现全屏图片的入场和出场动画
    enter (el, done) {
      // 定义全屏图片入场前的初始状态，另其处于迷你图的位置
  ...
  watch: {
    currentSong (newSong, oldSong) {
      // 当切换歌曲列表播放模式后，歌曲索引改变，会触发currentSong函数，
      // 导致歌曲暂停的情况下切换播放模式会使得歌曲重新播放，
      // 所以这里需要先验证当前歌曲是否为原来的歌曲
